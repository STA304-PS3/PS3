...
Because we are looking at factors that influence a woman’s choice to have children, we decided to look at only a subset of the data — only female respondents.  The large sample size and nature of GSS ensures that the sampled data represents the data well.  

To conduct the analysis, we decided to use logistic regression. Logistic regression works well for the response variable in our analysis (a categorical variable) and it also incorporates both numerical and categorical explanatory variables.  In our analysis, variables such as family income, importance of religion, education level, province, and population center are categorical variables; to accommodate these variables, we use dummy variables in order to be able to use logistic regression.  

After looking at the results from our logistic regression and using a significance level of 0.05, we inspected the p-values of the different variables to determine if the variables are significant. We were able to confirm: 

The results from our analysis could be useful in many different ways. Information about factors that influence a woman’s choice to have children can be used for information regarding population demographics. 
...
![Canadian Income Levels 2010](2010-Canada-Income.gif)
