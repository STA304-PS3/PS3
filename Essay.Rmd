...
In this report, we will be looking at the different factors than affect a woman’s choice whether or not to have children. After filtering the 2017 GSS (General Social Survey) dataset to only include variable related to our study, such as age, family income, education level and population centre, we ran a logistic regression-based model to identify the factors that affect a woman’s choice to have children. After completing our analysis, we were able to conclude that age, income, education, and region do affect a woman’s choice to have children. The results of our analysis are important because they assist in finding factors that influence birth rates which can aid in determining population demographics. 

## Introduction

Many factors can influence a woman’s choice on whether or not to have children and the information can be used in a multitude of ways. This poses the interesting question — what factors influence a woman’s choice to have children? 

In this report, we decided to investigate this question further by deeply analyzing the different factors that influence a woman’s choice to have children. Using the 2017 GSS (General Social Survey) dataset, we begun by identifying various variables that we thought could be influential in our analysis. In addition to obvious variables that provided us with information about the participant’s sex and whether the participant has had children, we decided to inspect the following variables: 
...
To conduct the analysis, we decided to use logistic regression.  In our analysis, variables such as family income, education level, and population center are categorical variables; to accommodate these variables, we use dummy variables in order to be able to use logistic regression.  

After looking at the results from our logistic regression and using a significance level of 0.05, we inspected the p-values of different variables to determine if the variables are significant. Age was determined to be a significant factor; income for those made an income less than 100,000 was determined to be significant; Population centered at PEI, rural and small population centers were also determined to be significant. 

The results from our analysis could be useful in many different ways. Information about factors that influence a woman’s choice to have children can be used for information related to population demographics. Countries with low birth rates can also find the results of our analysis to be useful as it can help them determine factors that can raise the birth rates. 

In this report, we will begin by discussing the dataset and then model our results. 
...
After observing the dataset, we noticed some immediate trends. One of the most prominent trends was that the data was skewed towards the older population. This is evident below: 
...
Clearly, in the above figure, we see an underrepresentation of the population aged younger than 50 compared to the population aged older than 50. This skewed distribution can result in severe bias in our results as it will overrepresent the population older than 50.  

Another trend we see in our data that can affect our results is in income. Below is a distribution of family income: 
...
It is evident in the figure above that our sample does contain a large proportion that has income greater than $125,000 which can result in bias in our results. 

### Weaknesses and Next Steps 

Using the GSS dataset from 2017 may lead to weaknesses in our dataset. Because we use a dataset that is exclusive to the year 2017, our data is limited and may be exposed to biases from that year. This may skew our results in a direction which might not be indicative of the true results. A solution to this problem is to survey participants over multiple years; this allows our data to contain more information and less likely to be biased

Another potential weakness of the dataset is that it only contains information on Canadian participants. This can be viewed as a weakness because it prevents our results from being generalizable to a different country. If we examined a different country that is not in North America, our results may differ drastically. A potential solution to this weakness is to gather information in different countries around the world, that way we have information on other countries and our results can be more generalizable. 

Since we are modeling what factors influence a woman’s choice to have children, it would be ideal to be able to collect longitudinal data. Because we are only collecting information at a certain point in time, our analysis may be missing out on certain influential factors. Collecting data from someone who is 80 years old will be drastically different than collecting data from someone who is in their 20s. The older our respondents are, the more likely they are to have already had a child at some point in their life.  A solution to this problem could be to collect longitudinal data so that we are comparing all respondents from the same perspective. Another solution in this case could be to conduct an in-depth interview rather than a survey. An in-depth interview ensures that we ask more detailed question and receive more detailed responses. Asking questions about when they made the decision to have children, and what their life situation was like at the time can provide much more meaningful information and more meaningful results. 

The dataset we used in the analysis has an average that is much older than the average Canadian population and also an average income that is quite high. To address this issue, a next step in this analysis can be to used post stratification. This allows our sample to more accurately reflect the population, making our results more meaningful. 
