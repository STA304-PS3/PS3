...
The dummy variables set ups are include in table 3, 4 and 5 in the appendix.
...
Table 2 summarises the results from our model by displaying the estimate of each variable, along with it's p-value:
...
The model predicts the following result for the probability a woman has children p, rounded to three decimal places. Since log is a one to one function with p, we say the change on $log(\frac{\hat{p}}{1-\hat{p}})$ is isomorphic to any change on p, the probability that a woman in Canada has children.
